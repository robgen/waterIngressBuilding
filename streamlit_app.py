        ...
        try:
            ...
            if enable_mc and uploaded_frag_ing is not None:
                ...
                try:
                    ...
                    _fpaths  = _frag.parse_ingress_fragility_file(_tmp_fi)
                    _fmems   = []
                    if uploaded_membrane:
                        _tmp_m = _save_tmp(_read_bytes(uploaded_membrane), '.csv')
                        _fmems = _frag.parse_membrane_file(_tmp_m)
...
with tab_results:
    ...
    if _r is None:
        ...
        st.divider()
        st.image(_r['sim_png_bytes'], use_container_width=True)
        ...
        if _r['anim_bytes']:
            ...
            st.image(_r['anim_bytes'], use_container_width=True)
...
with tab_diag:
    ...
    if _r is None:
        ...
        # ── Dashboard plot ─────────────────────────────────────────────────────
        st.image(_r['dash_bytes'], use_container_width=True)
...
with tab_batch:
    ...
    uploaded_batch = st.file_uploader(
        ...
        help='Output of parametric_run/run_parametric.py or similar.')
            ...
            if not _brows:
                ...
                with _bs_col_l:
                    ...
                    with open(_sc_p, 'rb') as _f:
                        _sc_b = _f.read()
                    st.image(_sc_b, use_container_width=True)
                    st.download_button(
                ...
                if _has_loss and _bs_col_r is not None:
                    with _bs_col_r:
                        ...
                        with open(_ls_p, 'rb') as _f:
                            _ls_b = _f.read()
                        st.image(_ls_b, use_container_width=True)
                        st.download_button(
